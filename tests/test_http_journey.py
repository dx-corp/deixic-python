    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:
            ...
            if method == "WatchEvents" and (
                ...
                self.end_headers()
                self.wfile.write(
                    b'{"code":"invalid_argument","message":"invalid Connect frame"}'
                )
                return
            if (
            ...
            if method == "SubmitTask":
                message = pb.SubmitTaskResponse(
                    ...
                    accepted_turn=pb.TaskTurn(
                        turn_id="turn-fixture", sequence=2, state=pb.TURN_STATE_ACCEPTED
                    ),
                )
                ...
                message = pb.ListEventsResponse(
                    ...
                    events=[
                        ...
                        pb.TaskEvent(
                            ...
                            id="own-progress",
                            turn_id="turn-fixture",
                            kind=pb.EVENT_KIND_PROGRESS,
                            text="Still working",
                        ),
                    ],
                )
            elif method == "WatchEvents":
                completed = True
                streaming = True
                ...
                message = pb.GetReceiptResponse(
                    receipt=pb.Receipt(
                        ...
                        state=pb.RECEIPT_STATE_VERIFIED,
                    )
                )
            else:
                self.send_error(404)
...
@pytest.mark.parametrize("expired_method", ["SubmitTask", "WatchEvents"])
def test_http_task_result_preserves_scope_and_replay_identity(
    expired_method: str,
) -> None:
    with loopback_owner(expired_method) as (base_url, calls, cursor):
        ...
        assert accepted.accepted_turn.state == pb.TURN_STATE_ACCEPTED
        early = client.threads.get(channel_id="company")
        assert early.turns[0].state == pb.TURN_STATE_RUNNING
        assert early.messages[0].body == "Still working"
        page = client.events.list(
            channel_id="company", after_cursor=accepted.replay_cursor
        )
        matching = [
            event
        ...
        assert [event.kind for event in matching] == [pb.EVENT_KIND_PROGRESS]
        assert page.next_cursor == cursor + 2
        stream = list(
            client.events.watch(channel_id="company", after_cursor=page.next_cursor)
        )
        assert stream[0].next_cursor == cursor + 3
        assert stream[0].events[0].turn_id == accepted.accepted_turn.turn_id
        assert stream[0].events[0].kind == pb.EVENT_KIND_TURN_COMPLETED
        final = client.threads.get(channel_id="company")
        assert final.turns[0].state == pb.TURN_STATE_COMPLETED
        ...
        receipt = client.receipts.get(
            channel_id="company",
            receipt_id=final.messages[0].receipt_ids[0],
        ).receipt
        assert receipt.state == pb.RECEIPT_STATE_VERIFIED
        ...
        for method, headers, body in calls:
            ...
            request_kind = getattr(pb, method + "Request")
            streaming = method == "WatchEvents"
            assert headers["Content-Type"] == (
                "application/connect+proto" if streaming else "application/proto"
            )
            request = request_kind.FromString(body[5:] if streaming else body)
            assert request.scope.organization_id == "org-fixture"
            assert request.scope.workspace_id == "ws-fixture"
            if method == "SubmitTask":
